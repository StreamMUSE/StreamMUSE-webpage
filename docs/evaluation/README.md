...
Each sample receives one **Accompaniment quality** integer score from 1 to 5, for three scores per round. Equal scores are allowed. A separate overall ranking places A/B/C first, second and third, using each sample exactly once without ties. The ranking is a direct listener choice, not calculated from scores. The question is how naturally and appropriately the accompaniment supports the melody. Every level in `src/lib/evaluation/rubric.ts` considers melody fit and whether repetition and changes respond to its development. Appropriate repetition is not penalized; more variation alone does not earn a higher score. The full guide appears once above the samples; selecting a score repeats its description below the buttons. This is a study-specific single criterion, not the earlier BEAT three-dimension rubric.

Migration `003_accompaniment_quality.sql` adds `evaluation_quality_responses` without changing the historical nine ratings or rankings in `evaluation_responses`. Migration `004_quality_ranking.sql` adds ranking to the quality response table; already-submitted quality-only rows keep SQL NULL for the uncollected ranking. The current API requires a complete ranking and rejects retries with changed scores or ranking. The scoring anchors, rubric version and browser keys remain unchanged when restoring ranking, so existing listeners keep their scores and progress. New APIs reject earlier rubric sessions with HTTP 410, and browser keys include both dataset and rubric versions. Listeners returning from the earlier three-dimension rubric start a new ten-song study; old drafts and answers are not converted, adopted or erased. The audio and dataset version are unchanged by this rubric update. Analyze cohorts separately by both `dataset_version` and `rubric_version`. Use a new version for future substantive changes to anchors or audio.
...
The business tables are `evaluation_participants`, `evaluation_sessions`, `evaluation_quality_responses` and the historical `evaluation_responses`. The migration runner adds `evaluation_schema_migrations` to track schema versions. A session stores song, dataset/rubric versions, actual A/B/C mapping, timestamps and participant/round fields. A current response stores three ratings, the overall ranking and the server submission time. SQL checks require exactly A/B/C with one integer `quality` value from 1–5 each; a non-null ranking must be a permutation of A/B/C. The column allows SQL NULL for earlier submissions and deployment compatibility; the current API requires ranking. The primary key and foreign key enforce one response per existing session. Old response constraints and rows remain intact. Apply all numbered migrations to both the target production database and any existing Preview database before deploying the new API.
...
| `POST /api/evaluations` | Accepts `sessionId`, `ratings` shaped as `{ "A": { "quality": 1 }, "B": { "quality": 3 }, "C": { "quality": 5 } }`, and `ranking` such as `["C", "A", "B"]` in first-to-third order. Validates and atomically inserts one answer. |
...
Each completed round offers **Listen to the next melody** until ten unique songs are complete. Every round independently selects seeds and shuffles A/B/C. The next round clears the three scores, ranking and audio positions, and focuses the reference player. The success screen shows completed progress and allows stopping and returning later. Participant and round identifiers are included in CSV export so repeated measures can be analyzed by listener.
...
The exporter performs a read-only join and writes one row per completed response, ordered by submission time. It includes participant UUID, round number, song, dataset/rubric, origin, timestamps, display labels, asset IDs, seeds, each system's quality score and overall rank. Historical response rows retain their three original dimension scores and ranking; current rows leave historical dimension columns blank, and historical rows leave quality blank. Quality-only submissions from before ranking was restored retain blank rank columns. Filter by rubric version before analysis. Unsubmitted sessions are excluded. CSV quoting and spreadsheet formula escaping are applied. Output files use owner-only permissions and an existing export is never overwritten. Store exports outside the public directory and Git repository. Use Neon SQL Editor for authenticated inspection and make manual backups periodically during collection.
...
Test the page against that database or a Neon Preview branch: submit three quality scores (including ties) and a complete ranking, reload midway and after submission, interrupt/retry a request, verify only one database row, and export that row. Check desktop/mobile and keyboard navigation, replay/seek and one-at-a-time audio. Do not use the production database for these tests.
