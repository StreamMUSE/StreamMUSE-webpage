...
export const headers = [
  'session_id', 'dataset_version', 'rubric_version', 'song_id', 'source_origin', 'created_at', 'submitted_at',
...
export function flattenResponse(row) {
  const result = Object.fromEntries(headers.slice(0, 7).map(key => [key, row[key] instanceof Date ? row[key].toISOString() : row[key]]))
